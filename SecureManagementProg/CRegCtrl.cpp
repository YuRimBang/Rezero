...
int CRegistry::RegDeleteKeyRecurse(HKEY hKey, LPCTSTR lpSubKey, char* svKeyBuf)
{
	int nCount = 0;
	...
	while (RegEnumKey(hSubKey, nCount, (LPWSTR)svKeyBuf, MAX_PATH) != ERROR_NO_MORE_ITEMS) {
		if (RegDeleteKeyRecurse(hSubKey, (LPWSTR)svKeyBuf, svSubKeyBuf) == -1) {
			RegCloseKey(hSubKey);
			return 1;
		}
		nCount++;
...
BOOL CRegistry::DeleteKey(CString strKey)
{
	...
	HKEY m_hKey = NULL;
	char svSubKeyBuf[MAX_PATH + 1];
	long m_RetValue = ::RegOpenKeyEx(m_hRootKey, strKey, 0, KEY_ALL_ACCESS, &m_hKey);
	if (m_RetValue == ERROR_SUCCESS) // 오픈 성공하면...
	{
		RegDeleteKeyRecurse(m_hRootKey, strKey, svSubKeyBuf);
	}
...
#ifdef _UNICODE
	//strcpy((char* )sz, LPSTR(LPCTSTR(strValue)));
#else
...
#ifdef _UNICODE
	if (::RegSetValueEx(hKey, LPCTSTR(strName), 0,
		REG_SZ, (LPBYTE)sz, strlen((char*)sz) + 1)
		!= ERROR_SUCCESS) bSuccess = FALSE;
#else
	if (::RegSetValueEx(hKey, LPCTSTR(strName), 0,
